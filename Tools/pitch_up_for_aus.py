...
# controller_port = param_dict["unreal_ground_dim_controller_port"]
mpix = param_dict["m_for_pix"]
axis_offset = []
stump = param_dict["stump"]
# centre_pitch_stumps_near_end = param_dict["center_pitch_stumps"]["near_end"]
# centre_pitch_stumps_far_end = param_dict["center_pitch_stumps"]["far_end"]
# axis_offset = [int(centre_pitch_stumps_near_end[0]+centre_pitch_stumps_far_end[0])/2,int(centre_pitch_stumps_near_end[1]+centre_pitch_stumps_far_end[1])/2]
...
def send_data_udp(message):
    try:
        print("here")
        serverSock.sendto(message.encode(
            'utf-8'), (param_dict["viz_udp_ip_address"], param_dict["viz_udp_port"]))
    ...
    if k == ord("w"): 
        ...
        for point in el_list_src:
            if point is not None and point !=[]:                
                ...
                # ue4_string+= "X=" + str(round((x - axis_offset[0])*mpix*100)) + ",Y=" + str(round((y - axis_offset[1])*mpix*100)) +",Z=0" +"$$"
                p0_count+=1
        string+=str(x1)+":"+str(y1)+":0_"
        # ue4_string+= "X=" + str(round((x1 - axis_offset[0])*mpix*100)) + ",Y=" + str(round((y1 - axis_offset[1])*mpix*100)) +",Z=0" 
        
        boundary=copy.deepcopy(string)
        boundary = boundary[0:-1]
        with open('boundary_updater.txt', 'w') as f:
            f.write(boundary)
        o_json_data = {
                        "outer_circle": o_data
                        }

        with open('../Settings/outer_boundary.json','w') as data :
            json.dump(o_json_data,data)

        ue4_boundary=copy.deepcopy(ue4_string)
        ue4_boundary = ue4_string[0:-1]
        with open('outer_boundary.txt', 'w') as f:
            f.write(ue4_string)
        ...
        with open('ue4_boundary.txt', 'w') as f:
        ...
        if src_x and src_y != None:
            print(center_stumps)
            if len(center_stumps)<2:
                center_stumps.append([src_x,src_y])
                cv.circle(src_copy,(src_x,src_y),1,(255,0,0),-1)
                u_src.append(copy.deepcopy(src_copy))
                flag = "center_pitch_stumps"

        if len(center_stumps) ==2:
            stump_data = {}
            stump_src = {}
            if center_stumps[0] is not None and center_stumps[0] != []:
                homo_pt = calculate_homography(center_stumps[0], cam_matrix)
                x1, y1 = (int(homo_pt[0][0][0]), int(homo_pt[0][0][1]))
                stump_data["near_end"] = [x1, y1]

            if center_stumps[1] is not None and center_stumps[1] != []:
                homo_pt = calculate_homography(center_stumps[1], cam_matrix)
                x2, y2 = (int(homo_pt[0][0][0]), int(homo_pt[0][0][1]))
                stump_data["far_end"] = [x2, y2]
            with open("../Settings/config.json", 'r') as f:
                config_dict = json.load(f)
            config_dict['center_pitch_stumps'] = stump_data
            with open("../Settings/config.json", 'w') as f:
                json.dump(config_dict, f, indent=4)
        
    # elif k ==ord("k"):
    #     #send to controller
    #     with open('../Settings/outer_boundary.txt') as f:
    #         lines = f.readlines()
    #     with open("../Settings/config.json","r") as f:
    #         param_dict = json.load(f)
    #     centre_pitch_stumps_near_end = param_dict["center_pitch_stumps"]["near_end"]
    #     centre_pitch_stumps_far_end = param_dict["center_pitch_stumps"]["far_end"]
    #     midpoint = [int(centre_pitch_stumps_near_end[0]+centre_pitch_stumps_far_end[0])/2,int(centre_pitch_stumps_near_end[1]+centre_pitch_stumps_far_end[1])/2]
            
    #     message = lines[0]
    #     print(lines[0])
    #     with open('ground_dim.txt', 'w') as f:
    #         f.write(message)
    #     send_data_udp(message)
        ...
        def undo(flag):
            ...
            if flag == "el_list_src":
                ...
                if gap_points:
                    ...
                    if len(u_src)!=0 and len(gap_points)>=1:
                        gap_points.pop()
                        u_src.pop()
                        src_copy = copy.deepcopy(u_src[-1])
                    flag = None
            ...
            elif flag == "center_pitch_stumps":
                if center_stumps:
                    cv.circle(src_copy,(center_stumps[-1][0],center_stumps[-1][1]),1,(255,255,255),-1)
                    print(center_stumps)
                    
                    if len(u_src)!=0 and len(center_stumps)>=1:
                        center_stumps.pop()
                        print(center_stumps)
                        u_src.pop()
                        src_copy = copy.deepcopy(u_src[-1])
